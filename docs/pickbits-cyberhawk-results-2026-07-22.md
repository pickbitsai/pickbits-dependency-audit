# A Redacted PickBits Dependency Audit Case Study

**Test date:** July 22, 2026<br>
**Target:** a redacted local Windows development portfolio<br>
**Mode:** read-only OSV import, lockfile admission audit, persistent local dashboard

We did not build a hand-picked demo repository. We ran PickBits Dependency Audit against an active development portfolio and asked three separate questions:
...
An early optional experiment intersected the result with that week's 19-item CyberHawk editorial brief and returned zero direct CVE-ID matches. That sentence has an important boundary: it means no IDs intersected, not that the folder was secure and not that every editorial item could be mapped perfectly to an ecosystem package and version. PickBits Dependency Audit uses OSV by default and does not require this editorial overlay.
...
`ALLOW_LOCKED` is intentionally narrow. It means the exact package version had an integrity value and resolved through the approved npm registry. It does **not** mean PickBits verified the publisher or certified the package as safe.
...
## Why this is a case study, not a scorecard

The figures come from two independently parsed artifacts: structured OSV-Scanner JSON and npm lockfile records. Finding occurrences are counted per project/package/advisory observation, so one CVE can appear more than once. Admission counts use lockfile records as their denominator and therefore must not be compared directly with OSV package occurrences. The generated report, SQLite state transitions, scanner digest, and test date provide the evidence trail; source paths and repository identities remain private.
